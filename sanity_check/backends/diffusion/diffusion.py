...
def DiffModelCIFAR():
    with open('sanity_check/backends/diffusion/configs/cifar10.yml', 'r') as f:
        config = yaml.safe_load(f)
    new_config = dict2namespace(config)
    return DiffModel(new_config)

def DiffModelBedroom():
    with open('sanity_check/backends/diffusion/configs/bedroom.yml', 'r') as f:
        config = yaml.safe_load(f)
    new_config = dict2namespace(config)
    return DiffModel(new_config)

def DiffModelCeleba():
    with open('sanity_check/backends/diffusion/configs/celeba.yml', 'r') as f:
        config = yaml.safe_load(f)
    new_config = dict2namespace(config)
    return DiffModel(new_config)
